...
const PRESUBMIT_SIGNATURE = '--- \n\n_autogenerated by AI-Review_';

function getCommitUrl(serverUrl: string, owner: string, repo: string, sha: string): string {
...
export function buildLoadingMessage(
  ...
  message += `Analyzing changes from base (\`${baseCommit.slice(
    0,
    7,
  )}\`) to latest commit (\`${commits[commits.length - 1].sha.slice(0, 7)}\`):\n`;
  ...
  for (const diff of fileDiffs) {
    ...
    fileText += ` _(${diff.hunks.length} ${diff.hunks.length === 1 ? 'hunk' : 'hunks'})_`;
    message += `${fileText}\n`;
  }
  message += '\n</details>\n\n';
...
export function buildOverviewMessage(summary: PullRequestSummary, commits: string[]): string {
